...
def flatten_record(obj: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in obj["pair"]:
        ...
        row = {
            ...
            "marker_family": obj["marker_family"],
            "lexical_domain": obj["lexical_domain"],
            "split": obj["split"],
            "type": pair_type,
            "sentence": item["sentence"],
...
def flatten_feature_dataset(input_dir: Path, pattern: str, output_path: Path, recursive: bool = True) -> pd.DataFrame:
    input_files = sorted(input_dir.rglob(pattern) if recursive else input_dir.glob(pattern))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help='Directory containing feature JSONL files. Default: "synthetic data".',
    )
    ...
    parser.add_argument(
        "--no-recursive",
        action="store_true",
        help="Do not recursively search input-dir.",
    )
    ...
    print()
    print("Marker families by split:")
    print(df.groupby("split")["marker_family"].nunique().sort_index().to_string())
    print()
    print("Lexical domains by split:")
    print(df.groupby("split")["lexical_domain"].nunique().sort_index().to_string())
