...
DEFAULT_CACHE_FILE_NAME = "xglm564m_feature_activations.pt"
...
def run_forward_passes(
    ...
    # Force right padding so final-token pooling selects the last real token.
...
def save_activation_cache(
    ...
    activation_path = output_dir / args.cache_file_name
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run model forward passes over flattened feature inputs."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/activation_cache/feature_dataset/xglm564m"),
    ...
    parser.add_argument(
        ...
        help="Activation cache filename.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Forward-pass batch size. XGLM-564M is larger than Pythia-70M; lower this if needed.",
    )
